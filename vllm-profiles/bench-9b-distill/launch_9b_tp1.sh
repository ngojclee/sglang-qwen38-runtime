#!/bin/bash
# P18: 9B TP1 (1×3090) — launch + KV + short bench
set -x
...
sleep 4
nohup env PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True CUDA_VISIBLE_DEVICES=0 \
  ...
  > /workspace/native_9b_tp1.log 2>&1 &
echo "LAUNCHED pid=$!"
echo "=== waiting health ==="
for i in $(seq 1 18); do
  sleep 20
  h=$(curl -sS -m 6 -o /dev/null -w '%{http_code}' http://127.0.0.1:18000/health 2>/dev/null)
  kv=$(grep -E 'GPU KV cache size|Maximum concurrency|Free memory' /workspace/native_9b_tp1.log 2>/dev/null | tail -3 | tr '\n' ' ')
  echo "chk $i: HEALTH=$h $kv"
  if [ "$h" = "200" ]; then echo READY; break; fi
  if grep -qE 'ValueError|RuntimeError|not valid|does not support' /workspace/native_9b_tp1.log 2>/dev/null; then echo LOG_ERROR; tail -10 /workspace/native_9b_tp1.log; break; fi
done
