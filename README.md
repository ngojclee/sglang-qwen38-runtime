...
| Tiêu chí Đánh giá | Máy A (2× 5060Ti vLLM) | Máy B (2× 5060Ti SGLang) | Máy C (4× 5060Ti DFlash2) | 📚 **Máy D (2× 3090 Baseline)** | 👑 **Máy D (2× 3090 FP8 0.90 Draft 8)** | 🏆 **Máy D (2× 3090 FP8 0.92 Draft 4 - Full 262K)** |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **KV Cache VRAM Capacity** | ~75K | ~40K | ~180K | **140.669 tokens** | 👑 **201.446 tokens** | 🏆 **224.560 tokens (Kỷ lục)** |
| **Test 1: Tốc độ Code Solo (tok/s)** | 35.0 tok/s | 40.4 tok/s | 75.20 tok/s | **52.25 tok/s** | 👑 **`62.05 – 91.53 tok/s`** | 🏆 **`56.18 tok/s`** |
| **Test 2: Mốc 32K Context** | ✅ PASS | ✅ PASS | ✅ PASS | ✅ **PASS (22.42s)** | ✅ **`PASS (23.73s)`** | ✅ **`PASS (24.36s)`** |
| **Test 2: Mốc 64K Context** | ✅ PASS | ❌ FAIL (40.9K) | ✅ PASS | ✅ **PASS (24.33s)** | ✅ **`PASS (26.07s)`** | ✅ **`PASS (78.27s)`** |
| **Test 2: Mốc 100K Context** | ⚠️ Drop Socket | ❌ FAIL | ✅ PASS | ✅ **PASS (30.17s)** | 🏆 **`PASS (32.63s - 78K tok)`** | 🏆 **`PASS (55.19s - 78K tok)`** |
| **Test 2: Mốc 150K Context** | ❌ FAIL | ❌ FAIL | ✅ PASS (180K VRAM) | ✅ **PASS (45.15s - HiCache)** | 🏆 **`PASS (48.36s - 117K tok)`** | 🏆 **`PASS (49.93s - 117K tok)`** |
| **Test 2: Mốc 200K Context** | ❌ FAIL | ❌ FAIL | ⚠️ HiCache RAM | ✅ **PASS (HiCache RAM)** | 👑 **`PASS (56.82s - 156K tok)`** | 👑 **`PASS (56.34s - 156K tok)`** |
| **Test 2: Mốc 262K Context** | ❌ FAIL | ❌ FAIL | ⚠️ HiCache RAM | ✅ **PASS (140K VRAM + HiCache)** | ⚠️ **201.4K VRAM + 33GB RAM** | 👑 **`PASS 100% (77.31s - 204K tok)`** |
| **Test 3: Tool Calling (`qwen3_coder`)**| ⚠️ Format JSON | ✅ PASS | ✅ PASS | ✅ **PASS (1.57s)** | ✅ **`PASS (1.52s)`** | ✅ **`PASS (1.72s)`** |
| **Chi phí thuê / giờ** | ~$0.20/h | ~$0.25/h | ~$0.85/h | 🥇 **~$0.30 – $0.45/h** | 🥇 **`~$0.30 – $0.45/h`** | 🥇 **`~$0.30 – $0.45/h (P/P Vô Địch)`** |

#### 🧠 Chi tiết Phân Bổ Dung Lượng Context Thực Tế:
1. **Cấu hình Vô Địch Toàn Diện (0.92 + Draft 4):** Cấp phát **224.560 tokens KV Cache FP8** trực tiếp trên VRAM. Vượt qua xuất sắc **cả 6 mốc thang đo (từ 32K đến FULL 262K Context)** với tốc độ sinh từ **56.18 tok/s** và gọi Tool Calling chỉ trong 1.72s.
2. **Cấu hình Tốc Độ Đỉnh Cao (0.90 + Draft 8):** Cấp phát **201.446 tokens FP8** trên VRAM. Tốc độ sinh từ đạt đỉnh **62.05 – 91.53 tok/s**, vượt hoàn hảo mốc **200K context (156K payload)** trong 56.82s.
3. **Cấu hình Mở Rộng 0.93 + Draft 2:** Cấp phát **232.537 tokens FP8** trên VRAM, vượt 262K context trong 77.52s, tốc độ **44.03 tok/s**.
4. **Cơ chế vận hành:** Tự động hoán đổi qua lại giữa các cấu hình bằng các profile script trong thư mục root.
...
> **Câu hỏi kiến trúc:** *Cần bao nhiêu VRAM để vừa bật DFlash2, vừa giữ trọn vẹn 262.144 tokens ngay trên VRAM không bị tràn OOM?*
...
#### 💡 Bảng Tổng Kết Thực Nghiệm Thang Đo Mem-Fraction & Draft Tokens Trên 2× RTX 3090:
| Mem-Fraction | Draft Tokens | VRAM KV Tokens | Free VRAM | Kết Quả Thực Nghiệm & Phân Tích Kỹ Thuật |
| :---: | :---: | :---: | :---: | :--- |
| **0.90** | **8** | **201.446 tok** | **2.45 GB** | 👑 **TỐC ĐỘ CAO NHẤT (91.5 tok/s)**. PASS 100% mốc 200K (56.82s). |
| **0.91** | **8** | **180.788 tok** | **0.99 GB** | ✅ **PASS 100% mốc 200K (57.51s)**. Tốc độ 62.80 tok/s, Tool call 1.75s. |
| **0.92** | **8 / 6** | **210.242 tok** | **52.5 MiB** | ❌ **OOM khi Prefill**: Marlin GEMM cần 68 MB activation ➔ Crash tiến trình. |
| **0.92** | **4** | **224.560 tok** | **0.78 GB** | 🏆 **VÔ ĐỊCH TOÀN DIỆN**: Giải phóng đủ VRAM cho Marlin GEMM. **PASS 100% FULL 262K CONTEXT (77.31s)**, Tốc độ 56.18 tok/s, Tool call 1.72s! |
| **0.93** | **2** | **232.537 tok** | **0.70 GB** | ✅ **PASS 100% FULL 262K CONTEXT (77.52s)**. Tốc độ 44.03 tok/s (chậm hơn vì draft=2). |
| **0.95** | **8** | - | **10.5 MiB** | ❌ **BOOT CRASH (OOM)**: `capture_decode_graph` cần 20 MB nhưng chỉ còn 10.5 MB. |
| **0.95** | **6** | - | **4.5 MiB** | ❌ **BOOT CRASH (OOM)**: `capture_decode_graph` cần 20 MB nhưng chỉ còn 4.5 MB. |

#### 🔑 Giải Thích Hiện Tượng "sglang: ERROR (not running)" Khi Restart:
Khi gõ lệnh `supervisorctl restart sglang`, supervisor sẽ thực hiện 2 bước:
1. `stop sglang`: Nếu trước đó tiến trình sglang đã **tự động sập (crash do OOM)** trong lúc capture CUDA graph, supervisorctl sẽ báo `sglang: ERROR (not running)` vì không còn tiến trình nào để tắt.
2. `start sglang`: Supervisorctl tiếp tục khởi chạy lại tiến trình mới theo đúng lệnh restart.
➔ Đây là dấu hiệu nhận biết 100% cấu hình trước đó (ở mức 0.95) đã bị crash do cạn kiệt VRAM.
